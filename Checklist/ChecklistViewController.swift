...
//  Checklist
//
//  Created by Others on 25.06.2019.
...
class ChecklistViewController: UITableViewController {
        ...
        if let selectedRows = tableView.indexPathsForSelectedRows{
            for indexPath in selectedRows{
                if let priority = priorityForSectionIndex(indexPath.section){
                    ...
                    let rowToDelete = indexPath.row > todos.count - 1 ? todos.count-1:
                        indexPath.row
    ...
    override func prepare(for segue: UIStoryboardSegue, sender: Any?) {
        if segue.identifier == "AddItemSegue" {
            if let itemDetailViewController = segue.destination as?
...
extension ChecklistViewController: ItemDetailViewControllerDelegate {
    ...
    func itemDetailViewController(_ controller: ItemDetailViewController, didFinishAdding item: ChecklistItem) {
        navigationController?.popViewController(animated: true)
        let rowIndex = todoList.todoList(for: .medium).count - 1        //todoList.todos.append(item)
